...
from utils.dataset_utils import load_john_doe_runs
...
ax_3d[0].set_xlabel('StumpsX')
ax_3d[0].set_ylabel('StumpsY')
...
ax_3d[1].set_xlabel('StumpsX')
ax_3d[1].set_ylabel('StumpsY')
...
for i in range(K):
    ax_3d[2].scatter(Xplot[:, 0], Xplot[:, 1], assign_[:, i], color=colors[i], s=1)
ax_3d[2].set_title("Assignment Plot")
ax_3d[2].set_xlabel('StumpsX')
ax_3d[2].set_ylabel('StumpsY')
...
for i in range(K):
    ax_3d[3].scatter(Xplot[:, 0], Xplot[:, 1], fmean_[:, i], color=colors[i], s=1)
ax_3d[3].set_title("Prediction Plot")
ax_3d[3].set_xlabel('StumpsX')
ax_3d[3].set_ylabel('StumpsY')
...
for i in range(2):
    if i == 0:
        ax[i + 1].set_title("StumpsY Constant Value = " + str(stumpsY_const_value))
        ax[i + 1].set_xlabel('StumpsX')
    elif i == 1:
        ax[i + 1].set_title("StumpsX Constant Value = " + str(stumpsX_const_value))
        ax[i + 1].set_xlabel('StumpsY')
...
for i in range(2):
    if i == 0:
        ax[i + 3].set_title("StumpsY Constant Value = " + str(stumpsY_const_value))
        ax[i + 3].set_xlabel('StumpsX')
    elif i == 1:
        ax[i + 3].set_title("StumpsX Constant Value = " + str(stumpsX_const_value))
        ax[i + 3].set_xlabel('StumpsY')
...
plt.tight_layout()
plt.show()
fig_3d.savefig('../figs/demo_' + name + "_".join(numerical_attributes) + '_1.png')
fig.savefig('../figs/demo_' + name + "_".join(numerical_attributes) + '_2.png')
